...
from recreated_data import make_datasets 
...
def load_german(recreated_data):
    ######### ADDITIONS #################
...
""" Added the conidition for the data folder and also added a parameter to the function """
def load_compas(recreated_data):
    ######### ADDITIONS #################
    if recreated_data == "yes" or recreated_data == "y":
        DATA_FOLDER = os.path.join(".", "recreated_data")  
    else:
...
def load_dataset(dataset_name, recreated_data, rand_seed):
    ...
    if recreated_data == "yes" or recreated_data == "y":
        if len([i for i in glob.glob(os.path.join("recreated_data", "*")) if i.split(os.path.join("recreated_data", " ").strip())[1] in dataset_names]) != len(dataset_names):
            make_datasets.recreate_german_dataset()
            make_datasets.recreate_compas_dataset()
            make_datasets.recreate_drug_dataset()
    ###############################################################

    if dataset_name == 'german':
        return load_german(recreated_data)
    elif dataset_name == 'compas':
        return load_compas(recreated_data)
    elif dataset_name == 'drug':
        return load_drug(recreated_data)